...
export const getColumns = (refetch: () => void): ColumnDef<Client>[] => [
  { accessorKey: "conversion_id", header: "Conversion ID" },
  { accessorKey: "uid", header: "UID" },
  { accessorKey: "from", header: "From" },
  { accessorKey: "to", header: "To" },
  { accessorKey: "amount", header: "Amount" },
  { accessorKey: "converted_amount", header: "Converted Amount" },
  {
    ...
    cell: ({ row }) => {
      ...
      async function updateStatus(id: string, status: "approved" | "rejected") {
        try {
          ...
          await axios.put(
            "https://api.fluxcoin.tech/api/v1/update-order?apikey=A20RqFwVktRxxRqrKBtmi6ud",
            { order_id: id, status },
            {
              headers: {
