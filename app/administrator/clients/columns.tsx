...
function ClientActions({ client }: { client: Client }) {
  ...
  const handleFreeze = async () => {
    try {
      ...
      await axios.put(
        "https://api.fluxcoin.tech/api/v1/freeze-account?apikey=5lPMMw7mIuyzQQDjlKJbe0dY",
        {
          uid: client.uid,
  ...
  const handleLevelSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await axios.put(
        "https://api.fluxcoin.tech/api/v1/update-level?apikey=5lPMMw7mIuyzQQDjlKJbe0dY",
        {
          uid: client.uid,
          level: selectedLevel,
        },
